...
fn simulate_falling_bytes(maze: &mut [Vec<char>], byte: (usize, usize)) {
    maze[byte.1][byte.0] = '#';
}
...
fn main() {
    ...
    // note its optimized for 70x70 so it wont work properly with 6x6
    let mut maze = vec![vec!['.'; 71]; 71];
    for next in bytes {
        simulate_falling_bytes(&mut maze, next);

        // cool visualization
        // maze.iter().for_each(|line| println!("{line:?}"));

        if a_star(&maze, start, goal).is_none() {
            // this is the answer
            println!("{next:?}");
            break;
        }
